#version 1.1
#takes the mean score instead of the original score for the purpsose of decision making. Does so by taking the score of a node and dividing it by the number of children.
import copy
global count
Score = 0
count = 0
        ...
        if self.IsEndState() == True:
            ...
            if self.IsRoot == False:
                for n in range(len(self.children)):
                    self.Score=self.Score + self.children[n].Score
                #divides the score by the number of children nodes, in order to get the mean score.
                self.Score = self.Score/len(self.children)
...
while Done == False:
    #player1 turn
    exampleboard = player1(exampleboard)
